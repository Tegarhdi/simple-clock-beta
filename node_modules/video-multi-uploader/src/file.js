...
utils.extend(File.prototype, {
  ...
  _eachAccess: function(eachFn, fileFn) {
    // console.log('resume 2 _eachAccess: isFolder:', this.isFolder, ',files:', this.files)
    if (this.isFolder) {
  ...
  _measureSpeed: function(hard) {
    // console.log('_measureSpeed hard:', hard)
    var averageSpeeds = 0
    var currentSpeeds = 0
    var num = 0
  ...
  _chunkEvent: function(chunk, evt, message) {
    ...
    var triggerProgress = function(hard) {
      ...
      console.log('_chunkEvent trigger start')
      uploader._trigger('fileProgress', rootFile, that, chunk)
      console.log('_chunkEvent trigger end')
    ...
    switch (evt) {
      ...
      case STATUS.SUCCESS:
        ...
        break
      case STATUS.RETRY:
        console.log('file.js fileRetry')
        uploader._trigger('fileRetry', rootFile, this, message, chunk)
        break
      case STATUS.REQUEST_REPORT:
        console.log('requestReport _trigger')
        uploader._trigger('requestReport', rootFile, this, message, chunk)
        break
    }
  },

  _updateUploadedChunks: function(message, chunk) {
    console.log('_updateUploadedChunks message:', message + 'chunk:', chunk)
  ...
  isUploading: function() {
    console.log('file.js isUploading')
    var uploading = false
    this._eachAccess(function(file) {
      if (file.isUploading()) {
        uploading = true
  ...
  sizeUploaded: function() {
    console.log('file.js sizeUploaded')
    var size = 0
    this._eachAccess(function(file) {
