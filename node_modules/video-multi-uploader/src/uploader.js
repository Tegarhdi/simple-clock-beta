...
utils.extend(Uploader.prototype, {
  ...
  uploadNextChunk: function(preventEvents) {
    ...
    if (this.opts.prioritizeFirstAndLastChunk) {
      utils.each(this.files, function(file) {
        console.log('uploadNextChunk prioritizeFirstAndLastChunk upload ...', file)
        if (file.paused) {
          return
        }
    ...
    utils.each(this.files, function(file) {
      console.log('uploadNextChunk simply look for the next upload ...', file)
      if (!file.paused) {
        console.log('uploadNextChunk not paused.')
        if (checkChunkUploaded && !file._firstResponse && file.isUploading()) {
          // waiting for current file's first chunk response
          console.log('uploadNextChunk not paused. waiting for current file\'s first chunk respons')
          return
        }
        console.log('uploadNextChunk not paused.  continue')
        utils.each(file.chunks, function(chunk) {
          if (chunk.status() === pendingStatus) {
            console.log('', chunk, new Date(), 'pendingStatus: ', pendingStatus + ',chunk.status():' + chunk.status())
            chunk.send()
            found = true
            return false
          }
  ...
  upload: function(preventEvents) {
    console.log('start upload....', preventEvents)
    // Make sure we don't start too many uploads at once
    var ret = this._shouldUploadNext()
    console.log('shouldUploadNext ret:', ret)
    if (ret === false) {
      console.log('shouldUploadNext ret: false')
      return
    }
    !preventEvents && this._trigger('uploadStart')
    var started = false
    for (var num = 1; num <= this.opts.simultaneousUploads - ret; num++) {
      console.log('upload num:', num, this.opts.simultaneousUploads)
    ...
    if (!preventEvents) {
      console.log('calOtherChunkMd5 ', preventEvents)
      this._trigger('calOtherChunkMd5')
    }
    console.log('upload send succeing ...')
    if (!started && !preventEvents) {
